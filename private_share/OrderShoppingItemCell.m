...
CGFloat const NameSize3 = 13.f;
CGFloat const PropertiesSize3 = 12.f;
CGFloat const ImageViewHeight3 = 64.f;

@implementation OrderShoppingItemCell {
    // basic element
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if(self) {
        ...
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(10, 5, ImageViewHeight3, ImageViewHeight3)];
...
+ (CGFloat)calcCellHeightWithShoppingItem:(ShoppingItem *)shoppingItem {
    if(shoppingItem != nil && shoppingItem.merchandise != nil) {
        ...
        if(![@"" isEqualToString:nameString] && ![@"" isEqualToString:propertiesString]) {
            CGSize nameSize = [shoppingItem.merchandise.name boundingRectWithSize:CGSizeMake(150, 160) options:(NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading) attributes:@{ NSFontAttributeName : [UIFont systemFontOfSize:NameSize3] } context:nil].size;
            CGSize propertiesSize =  [shoppingItem.propertiesAsString boundingRectWithSize:CGSizeMake(150, 160) options:(NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading) attributes:@{ NSFontAttributeName : [UIFont systemFontOfSize:PropertiesSize3] } context:nil].size;
            CGFloat allLabelHeight = 5 + nameSize.height + 5 + propertiesSize.height;
            if(allLabelHeight < 5 + ImageViewHeight3) {
                allLabelHeight = 5 + ImageViewHeight3;
            }
            return allLabelHeight + 20;
        }
    }
    return 5 + ImageViewHeight3 + 20;
}
